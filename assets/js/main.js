$(document).ready(function () {
    ...
    $('#processor').on('click', function () {
        ...
        //opgepakt
        $('#page').append('<a-entity id="processor-indicator" position="3.85 4.55 -3.6">' +
	...
	$('#koelblok').on('click', function () {
		if (stap == 1) {
			cursorClick();
			showModelOnCamera(this.id);
			$('#page').append('<a-entity id="koelblok-indicator" position="3.85 4.65 -3.6">' +
                '<a-plane class="indicator" height=".6" width=".7" color="yellow" rotation="-90 0 0"></a-plane>' +
	...
	$("#voeding").on("click", function() {
		if (stap == 3) {
			cursorClick();
			showModelOnCamera(this.id);
			$('#page').append('<a-entity id="voeding-indicator" position="-3.51 4.87 -2.45">' +
